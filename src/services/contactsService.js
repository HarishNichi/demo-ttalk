...
async function _contactList(callBackFun) {
  try {
    ...
    toast.error(error?.response?.data?.message || "Failed to fetch contacts",{position:"top-right"});
    callBackFun(false);
  }
}
...
async function _contactCreate(payload, callBackFun) {
  try {
    ...
    toast.error(error?.response?.data?.message || "Failed to create contact",{position:"top-right"});
    callBackFun(false);
  }
}
...
async function _contactUpdate(id, payload, callBackFun) {
  try {
    ...
    toast.error(error?.response?.data?.message || "Failed to update contact",{position:"top-right"});
    callBackFun(false);
  }
}
...
async function _contactDetail(id, callBackFun) {
  try {
    ...
    toast.error(error?.response?.data?.message || "Failed to fetch contact details",{position:"top-right"});
    callBackFun(false);
  }
}
